...
def _raise_system_not_supported():
    raise NotImplementedError(
        "SDK not supported on {} with architecture {} ({} bits)".format(
            _platform, _machine, "64" if _is_64_bits else "32"
        )
    )
...
fgt_POWER = make_enum("fgt_POWER", "POWER_OFF", "POWER_ON", "SLEEP")

fgt_TTL_MODE = make_enum(
    "fgt_TTL_MODE",
    "DETECT_RISING_EDGE",
    "DETECT_FALLING_EDGE",
    "OUTPUT_PULSE_LOW",
...
fgt_SENSOR_CALIBRATION = make_enum(
    "fgt_SENSOR_CALIBRATION", "NONE", "H2O", "IPA", "HFE", "FC40", "OIL"
)

fgt_VALVE_TYPE = make_enum(
    "fgt_VALVE_TYPE",
    "NONE",
    "MSwitch",
    "TwoSwitch",
...
class fgt_STRUCT(Structure):
    def __repr__(self):
        return os.linesep.join(
            [str(self.__class__)]
            + ["{}: {}".format(attr, getattr(self, attr)) for attr, data_type in self._fields_]
        )
...
class fgt_CONTROLLER_INFO(fgt_STRUCT):
    ...
    _fields_ = [("SN", c_ushort), ("Firmware", c_ushort), ("id", c_uint), ("type", c_uint)]
...
lib.fgt_get_pressureStatus.argtypes = [
    c_uint,
    POINTER(c_int),
    POINTER(c_ushort),
    POINTER(c_ubyte),
...
lib.fgt_get_sensorStatus.argtypes = [
    c_uint,
    POINTER(c_int),
    POINTER(c_ushort),
    POINTER(c_ubyte),
...
def fgt_create_simulated_instr(instr_type, serial, version, config):
    ...
    c_error = c_ubyte(
        lib.fgt_create_simulated_instr(
            c_int(int(instr_type)),
            c_ushort(serial),
            c_ushort(version),
            config_pointer,
            config_length,
...
def fgt_set_sensorCustomScale(sensor_index, a, b, c):
    ...
    c_error = c_ubyte(
        lib.fgt_set_sensorCustomScale(sensor_index, c_float(a), c_float(b), c_float(c))
    )
    return (c_error.value,)
...
def fgt_set_sensorCustomScaleEx(sensor_index, a, b, c, qmax):
    """"""
    c_error = c_ubyte(
        lib.fgt_set_sensorCustomScaleEx(
            c_uint(sensor_index), c_float(a), c_float(b), c_float(c), c_float(qmax)
        )
    )
    return (c_error.value,)
...
def fgt_get_pressureEx(pressure_index):
    ...
    c_error = c_ubyte(
        lib.fgt_get_pressureEx(c_uint(pressure_index), byref(pressure), byref(timestamp))
    )
...
def fgt_set_sensorRegulation(sensor_index, pressure_index, setpoint):
    ...
    c_error = c_ubyte(
        lib.fgt_set_sensorRegulation(
            c_uint(sensor_index), c_uint(pressure_index), c_float(setpoint)
        )
    )
    return (c_error.value,)
...
def fgt_get_sensorValue(sensor_index):
    ...
    c_error = c_ubyte(lib.fgt_get_sensorValue(c_uint(sensor_index), byref(sensorValue)))
...
def fgt_get_sensorValueEx(sensor_index):
    ...
    timestamp = c_ushort(0)
    c_error = c_ubyte(
        lib.fgt_get_sensorValueEx(c_uint(sensor_index), byref(sensorValue), byref(timestamp))
    )
...
def fgt_set_customSensorRegulation(measure, command, max_sensor_range, pressure_index):
    ...
    c_error = c_ubyte(
        lib.fgt_set_customSensorRegulation(
            c_float(measure), c_float(command), c_float(max_sensor_range), c_uint(pressure_index)
        )
    )
    return (c_error.value,)
...
def fgt_get_pressureRange(pressure_index):
    ...
    c_error = c_ubyte(
        lib.fgt_get_pressureRange(c_uint(pressure_index), byref(pressure_min), byref(pressure_max))
    )
...
def fgt_get_sensorRange(sensor_index):
    ...
    c_error = c_ubyte(
        lib.fgt_get_sensorRange(c_uint(sensor_index), byref(sensor_min), byref(sensor_max))
    )
...
def fgt_get_valveRange(valve_index):
    ...
    posMax = c_int(0)
    c_error = c_ubyte(lib.fgt_get_valveRange(c_uint(valve_index), byref(posMax)))
...
def fgt_set_pressureLimit(pressure_index, p_lim_min, p_lim_max):
    ...
    c_error = c_ubyte(
        lib.fgt_set_pressureLimit(c_uint(pressure_index), c_float(p_lim_min), c_float(p_lim_max))
    )
    return (c_error.value,)
...
def fgt_set_sensorRegulationResponse(sensor_index, response_time):
    ...
    c_error = c_ubyte(
        lib.fgt_set_sensorRegulationResponse(c_uint(sensor_index), c_uint(response_time))
    )
    return (c_error.value,)
...
def fgt_set_pressureResponse(pressure_index, response_mode):
    ...
    c_error = c_ubyte(lib.fgt_set_pressureResponse(c_uint(pressure_index), c_ubyte(response_mode)))
    return (c_error.value,)
...
def fgt_get_pressureStatus(pressure_index):
    ...
    detail = (c_char * 200)()
    c_error = c_ubyte(
        lib.fgt_get_pressureStatus(
            c_uint(pressure_index), byref(instr_type), byref(instr_sn), byref(information), detail
        )
    )
    return c_error.value, instr_type.value, instr_sn.value, information.value, detail.value.decode()
...
def fgt_get_sensorStatus(sensor_index):
    ...
    detail = (c_char * 200)()
    c_error = c_ubyte(
        lib.fgt_get_sensorStatus(
            c_uint(sensor_index), byref(instr_type), byref(instr_sn), byref(information), detail
        )
    )
    return c_error.value, instr_type.value, instr_sn.value, information.value, detail.value.decode()
...
def fgt_set_power(controller_index, power_state):
    ...
    c_error = c_ubyte(lib.fgt_set_power(c_uint(controller_index), c_ubyte(power_state)))
    return (c_error.value,)
...
def fgt_get_power(controller_index):
    ...
    c_error = c_ubyte(lib.fgt_get_power(c_uint(controller_index), byref(power_state)))
...
def fgt_set_digitalOutput(controller_index, port, state):
    ...
    c_error = c_ubyte(
        lib.fgt_set_digitalOutput(c_uint(controller_index), c_ubyte(port), c_ubyte(state))
    )
    return (c_error.value,)
...
def fgt_detect():
    ...
    return (
        n_instruments.value,
        list(filter(None, serial_numbers)),
        list(filter(None, instr_types)),
    )
...
def fgt_set_valvePosition(valve_index, position, direction, wait):
    ...
    c_error = c_ubyte(
        lib.fgt_set_valvePosition(
            c_uint(valve_index), c_int(position), c_int(direction), c_int(wait_int)
        )
    )
    return (c_error.value,)
...
def fgt_set_allValves(controller_index, module_index, position):
    c_error = c_ubyte(
        lib.fgt_set_allValves(c_uint(controller_index), c_uint(module_index), c_int(position))
    )
    return (c_error.value,)
...
def fgt_get_differentialPressureRange(sensor_index):
    ...
    pressure_max = c_float(0)
    c_error = c_ubyte(
        lib.fgt_get_differentialPressureRange(
            c_uint(sensor_index), byref(pressure_min), byref(pressure_max)
        )
    )
    return c_error.value, pressure_min.value, pressure_max.value
...
def fgt_get_differentialPressure(sensor_index):
    ...
    c_error = c_ubyte(lib.fgt_get_differentialPressure(c_uint(sensor_index), byref(pressure_diff)))
...
def fgt_get_absolutePressureRange(sensor_index):
    ...
    pressure_max = c_float(0)
    c_error = c_ubyte(
        lib.fgt_get_absolutePressureRange(
            c_uint(sensor_index), byref(pressure_min), byref(pressure_max)
        )
    )
    return c_error.value, pressure_min.value, pressure_max.value
...
def fgt_get_absolutePressure(sensor_index):
    pressure_diff = c_float(0)
    c_error = c_ubyte(lib.fgt_get_absolutePressure(c_uint(sensor_index), byref(pressure_diff)))
    return c_error.value, pressure_diff.value


def fgt_get_sensorBypassValve(sensor_index):
    state = c_ubyte(0)
    c_error = c_ubyte(lib.fgt_get_sensorBypassValve(c_uint(sensor_index), byref(state)))
    return c_error.value, state.value


def fgt_set_sensorBypassValve(sensor_index, state):
    c_error = c_ubyte(lib.fgt_set_sensorBypassValve(c_uint(sensor_index), c_ubyte(state)))
    return (c_error.value,)
...
def fgt_set_log_output_mode(output_to_file, output_to_stderr, output_to_queue):
    ...
    c_error = c_ubyte(
        lib.fgt_set_log_output_mode(
            c_ubyte(output_to_file), c_ubyte(output_to_stderr), c_ubyte(output_to_queue)
        )
    )
    return (c_error.value,)
