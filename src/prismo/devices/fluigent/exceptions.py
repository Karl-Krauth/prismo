...
def parse_error_data(
    function_name, channel_type, channel_index, c_error, instr_type, instr_sn, information, detail
):
    ...
    error_origin = "{} {} {} of type {}".format(
        function_name, channel_type, channel_index, instr_type_str
    )
...
def manage_generic_status(function_name, c_error):
    ...
    e = FGT_WARNING("{} error {} - {}".format(function_name, c_error, c_error_str))
    warn(e)
...
def manage_pressure_status(function_name, pressure_index):
    ...
    handled).
    """
    c_error, instr_type, instr_sn, information, detail = low_level.fgt_get_pressureStatus(
        pressure_index
    )
    parse_error_data(
        function_name,
...
def manage_sensor_status(function_name, sensor_index):
    ...
    handled).
    """
    c_error, instr_type, instr_sn, information, detail = low_level.fgt_get_sensorStatus(
        sensor_index
    )
    parse_error_data(
        function_name,
