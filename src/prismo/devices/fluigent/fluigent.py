...
class FlowController:
    ...
    def __getitem__(self, idx):
        return fgt_get_pressure(idx)

    def __setitem__(self, idx, value):
        fgt_set_pressure(idx, value)
...
class fgt_CHANNEL_INFO(object):
    ...
    def __repr__(self):
        ...
        return os.linesep.join([str(self.__class__)] + [self.formatattr(attr) for attr in fields])
...
def fgt_get_sensorValue(sensor_index, include_timestamp=False, get_error=_get_error):
    ...
    if include_timestamp:
        ...
        c_error = fgt_ERROR(c_error)
        return (
            (c_error, sensorMeasurement, timestamp) if get_error else (sensorMeasurement, timestamp)
        )
    else:
...
def fgt_set_valvePosition(
    valve_index, position, direction=fgt_SWITCH_DIRECTION.Shortest, wait=True
):
